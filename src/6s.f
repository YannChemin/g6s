...
c     igeom=0.....
      if (igeom.eq.0) then
        ! Set the geometry of the platform manually
        !read(iread,*) asol,phi0,avis,phiv,month,jday
        asol = solarzenithangledeg
        phi0 = solarazimuthangledeg
        avis = satellitezenithangledeg
        phiv = satelliteazimuthangledeg
        month = monthnumber
        jday = dayofthemonth
      end if
...
c
      ...
      if (igeom.eq.4) then
        ...
        tu = dechourutchhddd
        nc = numberofcolumns
        xlonan = equatorcrossingascendantlongitude
        hna = equatorcrossingascendantdechour
      ...
      if (igeom.eq.6) then
        ! Set the geometry of HRV (SPOT)
        !1006 read(iread,*) month,jday,tu,xlon,xlat
        month = monthnumber
        jday = dayofthemonth
        tu = dechourutchhddd
        xlon = centralpixellongitude 
        xlat = centralpixellatitude
      ...
      if (igeom.eq.7) then
        ! Set the geometry of TM (LANDSAT)
      ...
      if (igeom.gt.7) then
        ! igeom is not yet defined, stop the program
        write(*,'(a)') 'Error, this geometry is not yet defined' 
        write(*,'(a, i3)') 'igeom = ', igeom 
        error stop
      end if
...
C           compute ratios and transmissions
            if (rocave.lt.1E-09) then
                write(6,*) "error rodir is near zero"
                write(6,*) "could not proceed with coupling correction"
                rogbrdf=-1.
                goto 3333
            endif
...
c    s            ' diff=',rogbrdf-brdfints(mu,1)
        ...
        ! Return the atmospherically corrected reflectance
        outputpixelreflectance = rog
      endif
      stop
